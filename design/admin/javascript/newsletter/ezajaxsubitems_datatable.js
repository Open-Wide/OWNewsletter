var sortableSubitems = function() {
	...
	function initDataTable() {
		...
		var customCheckbox = function(cell, record, column, data) {
			var disabled = record.getData('can_edit') ? '' : 'disabled="disabled"';
			cell.innerHTML = '<input type="checkbox" class="ezsubitems_delete_checkbox" name="DeleteIDArray[]" value="' + record.getData('node_id') + '" ' + disabled + ' />';
		};
		var customMenu = function(cell, rec, column, data) {
			...
			a.on('click', function(e) {
				if (rec.getData('can_edit')) {
					ezpopmenu_showTopLevel(e, 'SubitemsContextMenu', {
						'%nodeID%': rec.getData('node_id'),
					...
					ezpopmenu_showTopLevel(e, 'SubitemsContextMenuReadOnly', {
						'%nodeID%': rec.getData('node_id'),
						'%objectID%': rec.getData('contentobject_id'),
						'%version%': rec.getData('version'),
						'%languages%': translationArray,
		...
		var tblOptsDialog = new YAHOO.widget.SimpleDialog("to-dialog-container", {
			width: "25em",
			visible: false,
			modal: true,
			buttons: [{
					text: labelsObj.TABLE_OPTIONS.button_close,
		...
		var selectItemsBtn = new YAHOO.widget.Button({
			type: "menu",
			id: "ezbtn-items",
		...
		var createNewBtn = new YAHOO.widget.Button({
			type: "menu",
			id: "ezbtn-new",
		...
		if (createGroups.length === 0)
			createNewBtn.set('disabled', true);
		...
		var moreActBtn = new YAHOO.widget.Button({
			type: "menu",
			id: "ezbtn-more",
		...
		var tableOptionsBtn = new YAHOO.widget.Button({
			label: labelsObj.ACTION_BUTTONS.table_options,
			id: "ezbtn-options",
			container: "action-controls",
